		    ...
		    newArr.push(
		        <div className='flex space-x-2 justify-between' key={i} >
		            ...
		            <div className='flex justify-between w-1/3 space-x-2'>
		                ...
		                <p className={((dWeek[i][1].diff > 0) ? 'text-green-500' : (dWeek[i][1].diff < 0) ? 'text-red-500' : 'text-gray-400') + ' font-bold w-full'}>{((dWeek[i][1].diff > 0) ? '+' : '')}{dWeek[1].diff}<strong className='text-xs'>min</strong></p>
		            </div>
		        </div>
		    )
